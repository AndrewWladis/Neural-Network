...
def train_neural_network(train_images, train_labels, test_images, test_labels,
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(train_images), batch_size):
            ...
            total_cost += np.sum((output_activations - targets) **2)
        # Compute training loss
        #_, train_predictions = forward_pass(
        #    train_images, weights_input_hidden, biases_hidden, weights_hidden_output, biases_output
        #)
        #train_loss = compute_loss(train_predictions, one_hot_encode(train_labels, output_size))
        # Compute test loss
        #_, test_predictions = forward_pass(
        #    test_images, weights_input_hidden, biases_hidden, weights_hidden_output, biases_output
        #)
        #test_loss = compute_loss(test_predictions, one_hot_encode(test_labels, output_size))
        cost = np.sqrt(total_cost / len(train_images))
        print(f"Epoch {epoch + 1}/{epochs}, Test Loss: {cost:.6f}")
...
#Get num of digits categorized correctly instead of cost
# play around with stretegy to reduce learning rate if cost goes up
# get % of correct classification
